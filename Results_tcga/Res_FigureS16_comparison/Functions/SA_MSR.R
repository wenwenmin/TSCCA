SA_MSR = function(Tensor, ku, kv, kw, niter=1000, flag=1, seed0=1){
  # Use the 3D Merit Function of Eq. (3) 
  ...
  for(k in 1:niter){
    ...
    if(flg==1){
      ...
      deta.E = MSR_energyFun(Tensor, sn) - MSR_energyFun(Tensor, s0)
    ...
    obj = c(obj, MSR_energyFun(Tensor,s0)); #print(obj)
  }
...
Modularity_energyFun = function(Tensor, s){
  subTensor = abs(Tensor[s$x1,s$x2,s$x3])
  obj = mean(subTensor)
}

MSR_energyFun = function(Tensor, s){
  subTensor = Tensor[s$x1,s$x2,s$x3]
  Dim = dim(subTensor)
  ...
  for(i in 1:Dim[1]){
    for(j in 1:Dim[2]){
      for(k in 1:Dim[3]){
        MSR = MSR + subTensor[i,j,k] + 2*a_IJK - 
          mean(subTensor[i,,]) - 
          mean(subTensor[,j,]) - 
          mean(subTensor[,,k])  
      }
