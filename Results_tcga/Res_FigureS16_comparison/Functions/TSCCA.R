TSCCA = function(Tensor, ku, kv, kw, J=1, Init_flag=1, niter=50, seed0=1, err=0.0001){
  ...
  for(j in 1:J){
    ...
    res = Initialize_TSCCA(Tensor,DIM,Init_flag,seed0)
    ...
    tim = proc.time() - ptm
    times_vec[j] = tim[1]; print(times_vec)
    
    d[j] = t(w)%*%z.w; d_iter[[j]] = temp_d
    U[,j]=u; V[,j]=v; W[,j]=w
    Tensor = update.tensor(Tensor,u,v,w,d[j])
  }
  return (list(U=U, V=V, W=W, d=d, d_iter=d_iter, times=times_vec))
}
