SNCP_TSCCAInit = function(tscca.out, Tensor, ku, kv, kw, J=1, niter=20){
  ...
  for(j in 1:J){
    ptm = proc.time()
    # ---------------------------------------------------------------------------
    # For comparative fairness, SNCP and TSCCA have the same input data
    res[[j]] = SNCP_rank1(Tensor, ku, kv, kw, Init_flag=1, niter)
